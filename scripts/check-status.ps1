...
Write-Host "Run ID: $($latestRun.id) | Status: $($latestRun.status) | Conclusion: $($latestRun.conclusion) | Commit: $($latestRun.head_commit.message)"

$httpCode = curl.exe -s -o NUL -w "%{http_code}" https://qnzjn.github.io/gkflsdl333/
Write-Host "Site HTTP Code:" $httpCode
